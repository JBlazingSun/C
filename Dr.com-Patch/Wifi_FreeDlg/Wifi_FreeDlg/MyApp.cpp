...
DWORD JysNT::EnumProcesses(LPCWSTR window,WCHAR *Name)
{
	...
	{
		::OutputDebugString(L"初始化错误，不支持该进程模块枚举，请重试\n"); 
...
BOOL JysNT::AnsiToUcs(const CStringA &strAnsi, CStringW &strUcs)
{
	// Ansi 转换为 Ucs
...
BOOL JysNT::UcsToAnsi(const CStringW &strUcs, CStringA &strAnsi)
{
	// Ucs 转换为 Ansi
